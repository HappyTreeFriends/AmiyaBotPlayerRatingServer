...
namespace AmiyaBotPlayerRatingServer.Controllers.MAAControllers
{
    ...
    public class MAATaskController : ControllerBase
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "普通账户")]
        [HttpGet("GetConnectionLatestScreenShotThumbnail/{id}")]
        public async Task<IActionResult> GetConnectionLatestScreenShotThumbnail(Guid id)
        {
            // 从JWT中提取用户ID
            var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier);
            if (userIdClaim == null)
            ...
            try
            {
                ...
                if (result.ImagePayloadThumbnail != null)
                {
                    return Ok(new
                    {
                        Image = Convert.ToBase64String(result.ImagePayloadThumbnail)
                    });
                }
